...
from trainModel.model import BertClassifier, BertClassifierReg, mlp_score
...
class TextDataset(torch.utils.data.Dataset):
    def __init__(self, encodings, edge, num_node, pseudo_edge = None, pseudo_score = None, neg_edge = None, subseq_length = None):
    ...
    def __getitem__(self, idx):
        ...
        if self.pseudo_edge != None:
            ...
            first_zero = find_first_zero_indices(torch.cat((attention_mask, self.encodings['attention_mask'][indice]), dim = 0))
            start_point = np.random.randint(0, first_zero + 1 - self.subseq_length) if first_zero > self.subseq_length else 0

            item = {
                'input_ids': input_ids[:, start_point:start_point + self.subseq_length],
                'attention_mask': attention_mask[:, start_point:start_point + self.subseq_length],
                'pseudo_input_ids': self.encodings['input_ids'][indice][:, start_point:start_point + self.subseq_length],
                'pseudo_attention_mask': self.encodings['attention_mask'][indice][:, start_point:start_point + self.subseq_length],
...
log_path = f'Logs/GLEM_train/{args.data_name}/LM/reg({args.regression})/iter({args.em_iter})/seed({args.seed})/{args.lm_model}_es({args.edge_split_seed})_alpha{args.alpha}_seed{args.seed}_testlollol.log'
init_path(log_path)
...
if args.lm_model == 'all-MiniLM-L6-v2':
    config = AutoConfig.from_pretrained('model/pre_train/all-MiniLM-L6-v2/')
...
train_dataset = TextDataset(X, train_edge, num_node, pesudo_edge, pseudo_score, subseq_length = subseq_length)
...
logging.info(f"Number of parameters: {parameter_counter(model)}, sub_sequece length: {subseq_length}")
...
for run in range(runs):
    ...
    init_seed(seed)
    if args.em_iter == 0:
        pretrained_model = AutoModel.from_pretrained(f"model/pre_train/{args.lm_model}/", config = config).to(device)
    ...
    for epoch in range(1, 1 + num_epoch):
        for i, batch in enumerate(DataLoader(train_dataset, batch_size, drop_last=True, shuffle = True)):
            ...
            with autocast():
                ...
                _, _, loss_labeld, loss_pseudo = model(input_ids, attention_mask, pseudo_input_ids, pseudo_attention_mask, pseudo_score, subseq_length)
            ...
            logging.info(get_memory()) if flag == True else None
            flag = False
            if args.em_iter == 0:
                scheduler.step()
        ...
        if (epoch + 1) % eval_step == 0:
            ...
            with torch.no_grad():
                ...
                inf_model.eval()
                train_pos_preds, train_neg_preds = test(train_val_dataset, eval_batch_size, inf_model, token_max_length)
                val_pos_preds, val_neg_preds = test(val_dataset, eval_batch_size, inf_model, token_max_length)
                test_pos_preds, test_neg_preds = test(test_dataset, eval_batch_size, inf_model, token_max_length)
